...
function get_address_balance_at_tx_index(address, skip, start_blockindex, start_toskip, cb) {
  if (settings.addresses.show_balance_history != true) {
    return cb(false);
  } else {
    if (start_blockindex == false || start_toskip == false) {
      var findQuery = {a_id: address};
      var skipNum = skip;
    } else {
      var findQuery = {a_id: address, blockindex: {$lte:Number(start_blockindex)}};
      var skipNum = Number(start_toskip);
    }
    AddressTx.aggregate([
      { $match: findQuery }, 
      { $sort: {blockindex: -1} },
...
module.exports = {
  ...
  get_address_txs_ajax: function(hash, start, length, start_blockindex, start_toskip, cb) {
    var totalCount = 0;
    AddressTx.find({a_id: hash}).countDocuments({}, function(err, count){
      if(err) {
        ...
        totalCount = count;
        get_address_balance_at_tx_index(hash, Number(start), start_blockindex, start_toskip, function(balance_sum) { 
          if (err) {
            return cb(err);
          } else {
            if (start_blockindex == false || start_toskip == false) {
              var findQuery = {a_id: hash};
              var skipNum = Number(start);
            } else {
              var findQuery = {a_id: hash, blockindex: {$lte:Number(start_blockindex)}};
              var skipNum = Number(start_toskip);
            }
            AddressTx.find(findQuery, null, { sort: {blockindex: -1}, skip: skipNum, limit: Number(length) }).exec(function (err, address_tx) {
              if (err) {
                return cb(err);
              } else {
                var txs = [];
