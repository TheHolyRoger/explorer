...
function get_address_balance_at_tx_index(address, skip, cb) {
      ...
      if(err) {
        ...
        AddressTx.find({a_id: hash}).countDocuments({}, function(err, count){
          if(err) {
            ...
            if (start > (count/2)) {
              reverse = true;
              skipValBalance = Number(start);
              skipVal = (totalCount-Number(start)-1-length);
              if (skipValBalance < 0){skipValBalance = 0;}
              if (skipVal < 0){skipVal = 0;}
            } else {
              skipValBalance = Number(start);
              skipVal = Number(start);
            }
            console.log(reverse);
            get_address_balance_at_tx_index(hash, skipValBalance, function(balance_sum) { 
              if (err) {
                return cb(err);
              } else {
                if (reverse == true) {
                ...
                AddressTx.aggregate([
                  { $match: {a_id: hash} }, 
                  { $sort: {blockindex: sortDirection} },
                  { $skip: skipVal },  
                  { $limit: Number(length) }, 
                  { $sort: {blockindex: -1} }
                ], function(err, address_tx) {
                  if (err) {
                    return cb(err);
                  } else {
                    var txs = [];
                    ...
                    if (settings.addresses.show_balance_history == true) {
                      balanceSum = balance_sum[0].balance;
                      if (reverse == true) {
                        // console.log(addressBal);
                        // console.log(balanceSum);
                        // var running_balance = addressBal - balanceSum + address_tx[0].amount;
                        var running_balance = balanceSum;
