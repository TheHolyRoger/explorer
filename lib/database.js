    ...
    if (tx != 'There was an error. Check your console.') {
      lib.prepare_vin(tx, function(vin) {
        lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
          var updateAddress_vins = -1;
          if (tx.confirmations >= settings.confirmations) {
            updateAddress_vins = vin.length;
          }
          lib.syncLoop(updateAddress_vins, function (loop) {
            var i = loop.iteration();
            update_address(nvin[i].addresses, blockheight, txid, nvin[i].amount, 'vin', function(){
              loop.next();
            });
          }, function(){
            var updateAddress_vouts = -1;
            if (tx.confirmations >= settings.confirmations) {
              updateAddress_vouts = vout.length;
            }
            lib.syncLoop(updateAddress_vouts, function (subloop) {
...
function delete_tx(one_txid, cb) {
  Tx.findOneAndRemove({txid: one_txid}, function(err, exists) {
    if(exists) {
      return cb(true);
    } else {
      return cb(false);
            ...
            lib.get_blockhash(block_height, function(blockhash){
              if (blockhash) {
                lib.get_block(blockhash, function(block) {
                  // if (block && block.confirmations >= settings.confirmations) {
                  if (block && block.confirmations >= 0) {
                    ...
                    // Just realised this won't actually fire here, since the orhpaned block hash is replaced in the daemon.
                    // We'll have to add a separate function loop to lookup past X blocks of TXs in the database
                    async.eachLimit(block.tx, task_limit_txs, function(txid, next_tx) {
                      delete_tx(txid, function(err) {
                        setTimeout( function(){
                          next_tx();
                        }, timeout);
                      });
